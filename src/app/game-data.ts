...
export function getSimpleGridGameData(date: Date = new Date(), hardMode: boolean = false): {
  ...
  const allChars = getOnePieceCharacters(hardMode).filter(char => {
    const keys = allowedKeys.filter(k => typeof char[k] === 'string' && char[k]);
    return keys.length >= 2;
  });
  const shuffled = [...allChars].sort(() => random() - 0.5);
  const picked = shuffled.slice(0, 9);
  const cells = picked.map(char => {
    const keys = allowedKeys.filter(k => typeof char[k] === 'string' && char[k]);
    // Pick two different random keys
    const idx1 = Math.floor(random() * keys.length);
    let idx2 = Math.floor(random() * (keys.length - 1));
    if (idx2 >= idx1) idx2++;
    const key1 = keys[idx1];
    const key2 = keys[idx2];
    const value1 = String(char[key1] ?? char.name);
    const value2 = String(char[key2] ?? char.name);
    return {
      attr1: key1 || 'name',
      value1: value1 || char.name,
      attr2: key2 || 'name',
      value2: value2 || char.name,
      answer: char.name,
      hints: char.hints
    };
  });
